...
class Gpos implements IScriptTable implements ILayoutTable {
	...
	static function parseLookup2(p:Parser):Lookup {
		...
		if (res.posFormat == 1) {
			...
			res.pairSets = p.parseList(() -> p.parseAtPointer(p -> p.parseList(() -> new PairSet(p.parseUShort(),
				p.parseValueRecordOfFormat(res.valueFormat1), p.parseValueRecordOfFormat(res.valueFormat2)))));
		} else if (res.posFormat == 2) {
