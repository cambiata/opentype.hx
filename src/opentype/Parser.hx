...
class Parser {
	...
	public static function byte(p:Parser):Int
		return p.parseByte();
	...
	public static function char(p:Parser):Int
		return p.parseChar();
	...
	public static function uShort(p:Parser):Int
	...
	public static function short(p:Parser):Int
		return p.parseShort();
	...
	//--------------------------------------------
	public function parseF2Dot14():Float {
		final v = data.readS16BE(offset + relativeOffset) / 16384;
		relativeOffset += 2;
		return v;
	}
	...
	public function parseValueRecordOfFormat(valueFormat:Int):ValueRecord {
		...
		if (valueFormat & 0x0001 != 0) {
			valueRecord.xPlacement = parseShort();
		}
		if (valueFormat & 0x0002 != 0) {
			valueRecord.yPlacement = parseShort();
		}
		if (valueFormat & 0x0004 != 0) {
			valueRecord.xAdvance = parseShort();
		}
		if (valueFormat & 0x0008 != 0) {
			valueRecord.yAdvance = parseShort();
		}
		// Device table (non-variable font) / VariationIndex table (variable font) not supported
		// https://docs.microsoft.com/fr-fr/typography/opentype/spec/chapter2#devVarIdxTbls
		if (valueFormat & 0x0010 != 0) {
			valueRecord.xPlaDevice = 0;
			parseShort();
		}
		if (valueFormat & 0x0020 != 0) {
			valueRecord.yPlaDevice = 0;
			parseShort();
		}
		if (valueFormat & 0x0040 != 0) {
			valueRecord.xAdvDevice = 0;
			parseShort();
		}
		if (valueFormat & 0x0080 != 0) {
			valueRecord.yAdvDevice = 0;
			parseShort();
		}
	...
	public static function coverage(parser:Parser):Coverage {
	...
	// https://www.microsoft.com/typography/OTSPEC/chapter2.htm
	public static function classDef(p:Parser):ClassDefinition {
	...
	public function parseScriptList():Array<ScriptRecord> {
		var p = parsePointer();
		return if (p != null) {
			p.parseElements(p -> new ScriptRecord(p.parseTag(),
				p.parseAtPointer(p -> new Script(p.parseAtPointer(p -> new LangSys(p.parseUShort(), p.parseUShort(), p.parseUShortList())),
					p.parseElements(p -> new LangSysRecord(p.parseTag(),
						p.parseAtPointer(p -> new LangSys(p.parseUShort(), p.parseUShort(), p.parseUShortList()))))))));
		} else [];
	}
	...
	function featureTable(p:Parser):FeatureTable {
		return new FeatureTable(p.parseTag(), p.parseAtPointer(feature));
	}

	function feature(p:Parser):Feature {
		return new Feature(p.parseUShort(), p.parseUShortList());
	}
...
class RecordDescription<T> {
	public function new(name:String, parseFn:Parser->T) {
		this.name = name;
