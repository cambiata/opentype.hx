...
class GposTable extends buddy.BuddySuite {
	...
	function parseLookup(lookupType:Int, subTableData:String):Gpos {
		final data = '00010000 000A 000C 000E'
			+ // header
			'0000'
			+ // ScriptTable - 0 scripts
			'0000'
			+ // FeatureListTable - 0 features
			'0001 0004'
			+ // LookupListTable - 1 lookup table
			'000'
			+ lookupType
			+ '0000 0001 0008'
			+ // Lookup table - 1 subtable
	...
	public function new() {
		describe('tables/gpos.hx', {
			...
			it('can parse lookup1 SinglePosFormat1', function() {
				...
				var gpos = parseLookup(1, data);
				trace(gpos.posFormat);
				trace(gpos.coverage);
				trace(gpos.features);
